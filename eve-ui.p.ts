...
var eveui_user_agent: string = eveui_user_agent || 'For source website, see referrer. For library, see https://github.com/quiescens/eve-ui/ r:' + `626ca17`;
...
namespace eveui {
	...
	export function format_item( item_id: string ): string {
		let item = cache_retrieve( '/v3/universe/types/' + item_id );
		let html: string = `<img src="${ eveui_imageserver( 'Type/' + item_id + '_64' ) }" class="float_right" />${ item.name }<br />${ item.description }<hr /><table class="whitespace_nowrap">`;
		html += `<tr><td>Approx price<td>${ format_number( market_retrieve( item_id ).average_price ) }<tr><td>&nbsp;`;
